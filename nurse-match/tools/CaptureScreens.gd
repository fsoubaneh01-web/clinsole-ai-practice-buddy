...
func _ready() -> void:
	...
	# The board offers a move after HINT_DELAY of stillness.
	await _capture("06_idle_hint", Board.HINT_DELAY + 0.6)

	GameStateManager.change_state(GameStateManager.State.PAUSED)
	await _capture("07_paused", 0.8)
	GameStateManager.change_state(GameStateManager.State.PLAYING)
	await _idle(0.4)

	# Kept consistent with the real star rule: 11 of level 3's 25 moves spare is
	# 44%, which is a three-star finish.
	GameStateManager.last_result = {
		"level_id": 3, "score": 1420, "stars": 3,
		"objective": "Clear 16 bandages", "moves_left": 11,
	}
	GameStateManager.change_state(GameStateManager.State.LEVEL_COMPLETE)
	await _capture("08_level_complete", 1.0)

	GameStateManager.change_state(GameStateManager.State.PLAYING)
	await _idle(0.4)
	GameStateManager.last_result = {
		"level_id": 3, "score": 640, "target": 15, "current": 9,
		"objective": "Clear 16 bandages",
	}
	GameStateManager.change_state(GameStateManager.State.GAME_OVER)
	await _capture("09_game_over", 1.0)
